...
import { X, Moon, Sun, ArrowLeft, BookOpen, BookMarked, FileText, Library } from "lucide-react";
...
export default function NavigationDrawer({
    ...
    const drawerContent = (
        <div
            dir="rtl"
            className={`fixed inset-0 z-[99999] isolate bg-background/98 backdrop-blur-2xl transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
                }`}
        >
            {/* Decorative Background Pattern */}
            {/* Decorative Background Pattern */}
            <div className="absolute inset-0 z-[-1] opacity-[0.05] pointer-events-none bg-primary"
                style={{
            ...
            <div className="flex flex-col h-full max-w-7xl mx-auto px-6 md:px-12 relative">
                ...
                <div className="flex items-center justify-between h-24 md:h-32 shrink-0 z-20 border-b border-border/50">
                    ...
                    >
                        <X size={32} strokeWidth={1} className="text-primary group-hover:text-primary-foreground transition-all duration-500 group-hover:rotate-90" />
                    </button>
                </div>
                ...
                <div className="flex-1 flex items-center justify-center w-full overflow-y-auto py-8 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    <div className="w-full max-w-5xl px-4">
                            ...
                            {menu.filter((item) => item.url).map((item, index) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        onClick={onClose}
                                        className={`group relative overflow-hidden rounded-2xl bg-card shadow-sm hover:shadow-xl border border-border hover:border-primary/40 transition-all duration-500 hover:-translate-y-1 ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                                            }`}
                        ...
                        {/* CTA & Footer Section */}
                        <div className={`flex flex-col items-center gap-8 transition-all duration-700 delay-300 ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
                            {/* Short Description */}
