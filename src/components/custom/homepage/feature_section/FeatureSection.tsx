...
export default function FeatureSection({ data, index }: FeatureSectionProps) {
  ...
  const normalizedImage = data.image
    ? normalizeImage(data.image as unknown as ImageType)
    : null;
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
        {/* 60/40 Split Layout */}
        <div
          className={`grid grid-cols-1 lg:grid-cols-5 items-center gap-12 lg:gap-16 ${isImageRight ? "" : "lg:flex-row-reverse"}`}
        >
          ...
          {/* Image - 60% (3/5) */}
          <div
            className={`lg:col-span-3 w-full ${!isImageRight ? "lg:order-1" : ""}`}
          >
            <motion.div style={{ y }}>
              {imageUrl && (
